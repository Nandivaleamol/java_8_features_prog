...
public class BookMain {
    public static void main(String[] args) {
       ...
       books.forEach(System.out::println);
        ...
        List<String> olderAuthor = bookList.stream().map(Book::getAuthor).filter(author -> author.getAge() >= 50).distinct().limit(15).map(Author::getSurname).map(String::toUpperCase).collect(Collectors.toList());
        ...
        List<Book> updatedPriceBooks = bookList.stream().peek(book -> book.setBookPrice(book.getBookPrice() * 1.10)).collect(Collectors.toList());

        updatedPriceBooks.forEach(System.out::println);
